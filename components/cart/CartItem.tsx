...
const CartItem = ({ product }: Props) => {
  ...
  const handleRemove = () => {
    if (process.browser) {
      ...
      const updatedItems = getUpdatedItems(cart.products, 0, product.cartKey);
  ...
  return (
    ...
    >
      ...
      <Link href="/products/[slug]" as={`/products/${product.slug}`}>
        <a>
          <Text color="brand.purple" marginBottom={{ base: "1rem", md: 0 }}>
            {product.name}
          </Text>
        </a>
      </Link>
      <Text marginBottom={{ base: "1rem", md: 0 }}>{toAUD(product.price)}</Text>
      <Flex
        width="100%"
